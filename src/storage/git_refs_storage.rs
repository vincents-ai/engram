...
use crate::entities::{
    Entity, EntityRegistry, EntityRelationship, GenericEntity, RelationshipFilter,
};
...
impl GitRefsStorage {
    ...
    fn store_entity_as_ref(&self, entity: &GenericEntity) -> Result<(), EngramError> {
        ...
        if entity.entity_type == "reasoning_event"
...
mod tests {
    ...
    fn test_reasoning_store_auto_emits_event() {
        ...
        assert_eq!(event_ids.len(), 1, "Should have auto-emitted one reasoning_event");

        let event_entity = storage.get(&event_ids[0], "reasoning_event").unwrap().unwrap();
        let event = crate::entities::ReasoningEvent::from_generic(event_entity).unwrap();
        assert_eq!(event.reasoning_id, reasoning.id);
        assert_eq!(event.event_type, crate::entities::ReasoningEventType::AutoStored);
    }
    ...
    fn test_knowledge_citation_increment_on_relationship() {
        ...
        assert!(stored_k.last_used_at.is_some(), "last_used_at should be set");
    }
}
