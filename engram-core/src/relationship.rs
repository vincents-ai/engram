...
impl EntityRelationship {
    ...
    pub fn validate_constraints(&self) -> Result<(), crate::error::EngramError> {
        ...
        if let Some(ref source_types) = self.constraints.source_types {
            if !source_types.contains(&self.source_type) {
                return Err(crate::error::EngramError::Validation(format!(
...
mod tests {
    ...
    fn test_relationship_validation() {
        ...
        assert!(!relationship.source_id.is_empty());
        assert!(!relationship.target_id.is_empty());

        // Test self-relationship (source_id == target_id)
        ...
        assert_eq!(self_rel.source_id, self_rel.target_id);
    }
